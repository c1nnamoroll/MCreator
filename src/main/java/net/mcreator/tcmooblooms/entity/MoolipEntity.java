...
public class MoolipEntity extends Animal implements IAnimatable {
	public static final EntityDataAccessor<Boolean> SHOOT = SynchedEntityData.defineId(MoolipEntity.class, EntityDataSerializers.BOOLEAN);
	public static final EntityDataAccessor<String> ANIMATION = SynchedEntityData.defineId(MoolipEntity.class, EntityDataSerializers.STRING);
	public static final EntityDataAccessor<String> TEXTURE = SynchedEntityData.defineId(MoolipEntity.class, EntityDataSerializers.STRING);
	...
	public MoolipEntity(PlayMessages.SpawnEntity packet, Level world) {
		this(TcmoobloomsModEntities.MOOLIP.get(), world);
	}
	...
	public void baseTick() {
		super.baseTick();
		MoolipOnEntityTickUpdateProcedure.execute(this.level, this.getX(), this.getY(), this.getZ(), this);
	...
	public AgeableMob getBreedOffspring(ServerLevel serverWorld, AgeableMob ageable) {
		MoolipEntity retval = TcmoobloomsModEntities.MOOLIP.get().create(serverWorld);
	...
	public static void init() {
		SpawnPlacements.register(TcmoobloomsModEntities.MOOLIP.get(), SpawnPlacements.Type.ON_GROUND, Heightmap.Types.MOTION_BLOCKING_NO_LEAVES,
	...
	private <E extends IAnimatable> PlayState movementPredicate(AnimationEvent<E> event) {
		if (this.animationprocedure.equals("empty")) {
			if ((event.isMoving() || !(event.getLimbSwingAmount() > -0.15F && event.getLimbSwingAmount() < 0.15F))
				...
				event.getController().setAnimation(new AnimationBuilder().addAnimation("animation.Moolip.Walk", EDefaultLoopTypes.LOOP));
				return PlayState.CONTINUE;
			}
			if (this.isSprinting()) {
				event.getController().setAnimation(new AnimationBuilder().addAnimation("animation.Moolip.Sprint", EDefaultLoopTypes.LOOP));
				return PlayState.CONTINUE;
			}
			event.getController().setAnimation(new AnimationBuilder().addAnimation("animation.Moolip.Idle", EDefaultLoopTypes.LOOP));
			return PlayState.CONTINUE;
		}
	...
	protected void tickDeath() {
		...
		if (this.deathTime == 20) {
			this.remove(MoolipEntity.RemovalReason.KILLED);
			this.dropExperience();
		}
	}
